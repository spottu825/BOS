...
/** Local-only session host. It never exposes the session password in a URL or QR code. */
object LocalSessionServer {
    private const val PORT = 8080
    private var engine: ApplicationEngine? = null

    data class Session(val id: String, val url: String)
    ...
    fun start(password: String): Session {
        ...
        val path = "/s/$sessionId"
        engine = embeddedServer(CIO, host = "0.0.0.0", port = PORT) {
            routing {
                get(path) {
                    call.respondText(viewerPage(path), ContentType.Text.Html)
                }
                post("$path/unlock") {
        ...
        return Session(sessionId, "http://${localIpv4Address()}:$PORT$path")
    }
    ...
    private fun viewerPage(path: String) = """
        ...
        <form method="post" action="$path/unlock"><input name="password" type="password" autocomplete="current-password" required placeholder="BOS password"><button type="submit">Connect</button></form>
        <p><small>Local session only. Screen video transport is being enabled in the next BOS build.</small></p></main></body></html>
    """.trimIndent()
